  ...
  <div class="content">
    <div class="table all">
      ...
      <div class="center">
        ...
        <template v-for="(group, groupIndex) in webgroup" :key="group.description">
          <div class="web-group">
            <div class="web-des" draggable="true" @dragstart="dragStart($event, groupIndex)"
              @dragover="dragOver($event)" @dragleave="dragLeave($event)" @drop="drop($event, groupIndex)">
              {{ group.description }}
            </div>
            <div class="webs">
              <ul>
                <li class="li-drag" v-for="(web, webIndex) in group.webs"
                  :key="`${group.description}-${web.url}-${webIndex}`" draggable="true"
                  @dragstart="dragStart($event, groupIndex, webIndex)" @dragover="dragOver($event)"
                  @dragleave="dragLeave($event)" @drop="drop($event, groupIndex, webIndex)">
                  <img class="web-fav" :src="'http://www.google.com/s2/favicons?domain=' + web.url">
                  <a :href="web.url" target="_blank">{{ web.description }}</a>
                  <span class="icon-pencil opacity" title="修改" v-if="isEdit"
                    @click="showModifyModal('update', groupIndex, webIndex)"></span>
    ...
    <div id="modifyModal" v-show="isModifyOpen" class="modal">
      <div class="modal-content">
        ...
        选择分类：<input id="group_description" class="input_normal" type="text" list="group_list"
          v-model="modifyWeb.groupDescription" />
        <br>
        <br>
...
.li-drag {
  min-width: (@c-width * 90 / 600 - @m-r);
  height: 21px;
  margin: 10px @m-r 10px @m-l;
  cursor: move;
}
